CREATE PROCEDURE [dbo].[sp_report_DailyFlashERVisits]
    (
        @divisionId int = 0,
        @reportDate DATETIME,
		@qtd BIT = 0
...
BEGIN
	...
	ELSE IF @qtd = 1
		BEGIN
			...
			set s.er_visits = (
								...
								where coid = s.coid
									and year = datepart(yyyy, @reportDate)
									AND DATEPART(q, ER_VISITS_DATE) = DATEPART(q, @reportDate)
							)
			from @summary s;      
		END;      
	...
	declare @er_visits int

	declare cur cursor fast_forward read_only for
	select coid, flash_rollup_to_coid, er_visits
	from @summary s
	...
	fetch next from cur
	into @coid, @flash_rollup_to_coid, @er_visits

	while @@fetch_status = 0
		begin
			update @summary
			set er_visits = er_visits + @er_visits
