CREATE PROCEDURE [dbo].[sp_report_DailyFlashAdmissions]
    (
        @divisionId int = 0,
        @reportDate datetime
    )
...
BEGIN
    ...
    set s.admissions =  (
                            ...
                            from DailyAdmissions da WITH (NOLOCK)
								JOIN Facility f WITH (NOLOCK)
									ON da.COID = f.COID
                            where f.group_coid = s.coid
	...
	declare @admissions int

	declare cur cursor fast_forward read_only for
	select coid, flash_rollup_to_coid, admissions
	from @summary s
	...
	fetch next from cur
	into @coid, @flash_rollup_to_coid, @admissions

	while @@fetch_status = 0
		begin
			update @summary
			set admissions = admissions + @admissions
